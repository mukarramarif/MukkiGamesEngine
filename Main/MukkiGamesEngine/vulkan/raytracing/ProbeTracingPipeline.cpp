#include "ProbeTracingPipeline.h"
...
ProbeTracingPipeline::ProbeTracingPipeline() = default;
ProbeTracingPipeline::~ProbeTracingPipeline() { cleanup(); }

void ProbeTracingPipeline::init(Device* deviceIn)
{
    device = deviceIn;
...
void ProbeTracingPipeline::cleanup()
{
    if (!device) return;
...
VkDeviceAddress ProbeTracingPipeline::getBufferDeviceAddress(VkBuffer buffer) const
{
...
VkShaderModule ProbeTracingPipeline::createShaderModule(const std::vector<char>& code)
{
    ...
    if (vkCreateShaderModule(device->getDevice(), &createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
        throw std::runtime_error("failed to create probe tracing shader module!");
    }
    return shaderModule;
}

void ProbeTracingPipeline::createPipeline(VkDescriptorSetLayout descriptorSetLayout)
{
    auto rgenCode  = EngineUtils::readFile("Shaders/probeTrace.rgen.spv");
    auto rmissCode = EngineUtils::readFile("Shaders/probe.rmiss.spv");
    auto rchitCode = EngineUtils::readFile("Shaders/probe.rchit.spv");
    ...
    if (vkCreatePipelineLayout(device->getDevice(), &layoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS) {
        throw std::runtime_error("failed to create probe tracing pipeline layout!");
    }
    ...
    if (vkCreateRayTracingPipelinesKHRFunc(device->getDevice(), VK_NULL_HANDLE, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline) != VK_SUCCESS) {
        throw std::runtime_error("failed to create probe tracing pipeline!");
    }
...
void ProbeTracingPipeline::createShaderBindingTable()
{
    ...
    // (closest-hit only, so every SBT record holds a single handle)
    constexpr uint32_t groupCount = 3;
    const std::array<uint32_t, groupCount> handleCounts = { 1u, 1u, 1u };
    ...
    constexpr uint32_t totalHandleCount = 3;  // raygen + miss + closest
    ...
    if (vkGetRayTracingShaderGroupHandlesKHRFunc(device->getDevice(), pipeline, 0, groupCount,
        ...
        throw std::runtime_error("failed to get probe shader group handles");
    }

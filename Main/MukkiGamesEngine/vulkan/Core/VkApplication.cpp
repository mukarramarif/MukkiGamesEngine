...
VulkanApplication::VulkanApplication()
    : window(nullptr), device(nullptr), swapChain(nullptr),
      renderPassObj(nullptr), renderPass(VK_NULL_HANDLE),
      graphicsPipeline(nullptr), commandBufferManager(nullptr), currentFrame(0),
      vertexBuffer(VK_NULL_HANDLE), vertexBufferMemory(VK_NULL_HANDLE),
      indexBuffer(VK_NULL_HANDLE), indexBufferMemory(VK_NULL_HANDLE),
      indexCount(0), descriptorBoss(nullptr), textureManager(nullptr),
      bufferManager(nullptr), depthImage(VK_NULL_HANDLE),
      depthImageMemory(VK_NULL_HANDLE), depthImageView(VK_NULL_HANDLE),
...
void VulkanApplication::updateRayTracingUniformBuffer() {
  ...
  // Compute TAA jitter using Halton sequence
  uint32_t sampleIndex = (rtFrameCounter % 16u) + 1u;
  taaJitter.x = halton(sampleIndex, 2u) - 0.5f;
  taaJitter.y = halton(sampleIndex, 3u) - 0.5f;
  rtFrameCounter++;
...
struct RayTracingCameraUBO {
  ...
  model = glm::rotate(model, glm::radians(modelTransform.rotation.x),
                      glm::vec3(1.0f, 0.0f, 0.0f));
  model = glm::rotate(model, glm::radians(modelTransform.rotation.y),
                      glm::vec3(0.0f, 1.0f, 0.0f));
  model = glm::rotate(model, glm::radians(modelTransform.rotation.z),
...
void VulkanApplication::initVulkan()
{
  ...
  // 6. Create render pass (defines how rendering operations are performed)
  renderPassObj =
      new VulkanRenderPass(device, swapChain->getSwapChainImageFormat());
  renderPass = renderPassObj->getRenderPass();
  ...
  skybox->init(device, textureManager, bufferManager, renderPass,
               fullSkyboxPath, CubemapLayout::VerticalCross,
  ...
  m_deletionQueue.pushDescriptorSetLayout(device->getDevice(),
                                          descriptorSetLayout);
  m_deletionQueue.pushDescriptorSetLayout(device->getDevice(),
...
void VulkanApplication::createMaterialUniformBuffers() {
  ...
  for (size_t matIndex = 0; matIndex < materialCount; matIndex++) {
    ...
    materialData.metallicFactor =
        loadedModel.materials[matIndex].metallicFactor;
    materialData.roughnessFactor =
        loadedModel.materials[matIndex].roughnessFactor;
    ...
    for (size_t frame = 0; frame < MAX_FRAMES_IN_FLIGHT; frame++) {
      ...
      vkMapMemory(device->getDevice(),
                  materialUniformBuffersMemory[matIndex][frame], 0, bufferSize,
                  0, &materialUniformBuffersMapped[matIndex][frame]);
      memcpy(materialUniformBuffersMapped[matIndex][frame], &materialData,
             sizeof(MaterialUBO));
    }
  }
}
...
void VulkanApplication::cleanupMaterialUniformBuffers() {
  for (size_t matIndex = 0; matIndex < materialUniformBuffers.size();
       matIndex++) {
    for (size_t frame = 0; frame < materialUniformBuffers[matIndex].size();
         frame++) {
      if (materialUniformBuffers[matIndex][frame] != VK_NULL_HANDLE) {
        vkDestroyBuffer(device->getDevice(),
                        materialUniformBuffers[matIndex][frame], nullptr);
      ...
      if (materialUniformBuffersMemory[matIndex][frame] != VK_NULL_HANDLE) {
        vkFreeMemory(device->getDevice(),
                     materialUniformBuffersMemory[matIndex][frame], nullptr);
...
void VulkanApplication::updateUniformBuffer(uint32_t currentImage) {
  ...
  // Normal matrix (inverse transpose of model matrix for correct normal
  // transformation)
...
void VulkanApplication::cleanup() {
  ...
  // --- Swapchain-dependent resources (managed manually, recreated in
  // recreateSwapChain) ---
    ...
    for (size_t frame = 0; frame < MAX_FRAMES_IN_FLIGHT; frame++) {
      ...
      if (material.baseColorTextureIndex >= 0 &&
          material.baseColorTextureIndex <
              static_cast<int32_t>(loadedModel.textures.size())) {
        const LoadedTexture &tex =
            loadedModel.textures[material.baseColorTextureIndex];
...
void VulkanApplication::createComputeOutputImage() {
  ...
  if (vkCreateImage(device->getDevice(), &imageInfo, nullptr,
                    &computeOutputImage) != VK_SUCCESS) {
    throw std::runtime_error("failed to create compute output image!");
  }

  VkMemoryRequirements memRequirements;
  vkGetImageMemoryRequirements(device->getDevice(), computeOutputImage,
                               &memRequirements);
  ...
  if (vkAllocateMemory(device->getDevice(), &allocInfo, nullptr,
                       &computeOutputImageMemory) != VK_SUCCESS) {
    throw std::runtime_error("failed to allocate compute output image memory!");
  }

  vkBindImageMemory(device->getDevice(), computeOutputImage,
                    computeOutputImageMemory, 0);
  ...
  if (vkCreateImageView(device->getDevice(), &viewInfo, nullptr,
                        &computeOutputImageView) != VK_SUCCESS) {
...
void VulkanApplication::recordRayTracingCommandBuffer(
  ...
  uint32_t sbtStride = (sbt.handleSizeAligned + sbt.baseAlignment - 1) &
                       ~(sbt.baseAlignment - 1);
  ...
  // ---- TAA resolve pass ----
  // Execution barrier: ensure RT writes are visible to TAA compute shader
  VkMemoryBarrier rtToTAABarrier{};
  rtToTAABarrier.sType = VK_STRUCTURE_TYPE_MEMORY_BARRIER;
  rtToTAABarrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
  rtToTAABarrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;

  vkCmdPipelineBarrier(commandBuffer,
                       VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
                       VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1,
                       &rtToTAABarrier, 0, nullptr, 0, nullptr);

  // Bind TAA compute pipeline and descriptor set
  vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, taaPipeline);
  vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE,
                          taaPipelineLayout, 0, 1, &taaDescriptorSet, 0,
                          nullptr);

  // Push constants for TAA
  TAAPushConstants taaPC{};
  taaPC.resolution[0] = static_cast<float>(extent.width);
  taaPC.resolution[1] = static_cast<float>(extent.height);
  taaPC.blendFactor =
      0.05f; // small blend factor = strong temporal accumulation
  taaPC.isFirstFrame = taaFirstFrame ? 1u : 0u;
  taaFirstFrame = false;

  vkCmdPushConstants(commandBuffer, taaPipelineLayout,
  ...
  // Barrier: TAA output image GENERAL -> TRANSFER_SRC for copy to swapchain and
  // history
  VkImageMemoryBarrier taaOutputBarrier{};
  taaOutputBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
  taaOutputBarrier.oldLayout = VK_IMAGE_LAYOUT_GENERAL;
  taaOutputBarrier.newLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
  taaOutputBarrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
  taaOutputBarrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
  taaOutputBarrier.image = taaOutputImage;
  taaOutputBarrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  taaOutputBarrier.subresourceRange.baseMipLevel = 0;
  taaOutputBarrier.subresourceRange.levelCount = 1;
  taaOutputBarrier.subresourceRange.baseArrayLayer = 0;
  taaOutputBarrier.subresourceRange.layerCount = 1;
  taaOutputBarrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
  taaOutputBarrier.dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT;

  // Barrier: history image GENERAL -> TRANSFER_DST to receive the output copy
  VkImageMemoryBarrier taaHistoryBarrier{};
  taaHistoryBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
  taaHistoryBarrier.oldLayout = VK_IMAGE_LAYOUT_GENERAL;
  taaHistoryBarrier.newLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
  taaHistoryBarrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
  taaHistoryBarrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
  taaHistoryBarrier.image = taaHistoryImage;
  taaHistoryBarrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  taaHistoryBarrier.subresourceRange.baseMipLevel = 0;
  taaHistoryBarrier.subresourceRange.levelCount = 1;
  taaHistoryBarrier.subresourceRange.baseArrayLayer = 0;
  taaHistoryBarrier.subresourceRange.layerCount = 1;
  taaHistoryBarrier.srcAccessMask = VK_ACCESS_SHADER_READ_BIT;
  taaHistoryBarrier.dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;

  std::array<VkImageMemoryBarrier, 2> taaBarriers = {taaOutputBarrier,
                                                     taaHistoryBarrier};
  vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                       VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0,
                       nullptr, static_cast<uint32_t>(taaBarriers.size()),
                       taaBarriers.data());

  // Copy TAA output -> history (for next frame)
  VkImageCopy historyCopyRegion{};
  historyCopyRegion.srcSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  historyCopyRegion.srcSubresource.layerCount = 1;
  historyCopyRegion.dstSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  historyCopyRegion.dstSubresource.layerCount = 1;
  historyCopyRegion.extent.width = taaImageExtent.width;
  historyCopyRegion.extent.height = taaImageExtent.height;
  historyCopyRegion.extent.depth = 1;

  vkCmdCopyImage(commandBuffer, taaOutputImage,
                 VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, taaHistoryImage,
                 VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &historyCopyRegion);

  // Barrier: history image TRANSFER_DST -> GENERAL (ready for next frame's TAA
  // read)
  taaHistoryBarrier.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
  taaHistoryBarrier.newLayout = VK_IMAGE_LAYOUT_GENERAL;
  taaHistoryBarrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
  taaHistoryBarrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;

  vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                       VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 0, nullptr, 0,
                       nullptr, 1, &taaHistoryBarrier);
  ...
  // Copy TAA output -> swapchain image
  VkImageCopy copyRegion{};
  copyRegion.srcSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  copyRegion.srcSubresource.layerCount = 1;
  copyRegion.dstSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  copyRegion.dstSubresource.layerCount = 1;
  copyRegion.extent.width = taaImageExtent.width;
  copyRegion.extent.height = taaImageExtent.height;
  copyRegion.extent.depth = 1;
  ...
  // Barrier: swapchain TRANSFER_DST -> COLOR_ATTACHMENT for UI render pass
  swapBarrier.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
  swapBarrier.newLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
  swapBarrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
  swapBarrier.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
  ...
  // Barrier: TAA output TRANSFER_SRC -> GENERAL (ready for next frame's TAA
  // write)
  taaOutputBarrier.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
  taaOutputBarrier.newLayout = VK_IMAGE_LAYOUT_GENERAL;
  taaOutputBarrier.srcAccessMask = VK_ACCESS_TRANSFER_READ_BIT;
  taaOutputBarrier.dstAccessMask = VK_ACCESS_SHADER_WRITE_BIT;

  vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                       VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 0, nullptr, 0,
                       nullptr, 1, &taaOutputBarrier);

  // computeOutputImage stays in GENERAL layout throughout (RT writes, TAA
  // reads)
  ...
  if (modelLoaded && !modelDescriptorSets.empty() && additivePipeline) {
    ...
    vkCmdBindIndexBuffer(commandBuffer, loadedModel.indexBuffer, 0,
      ...
      for (const auto &primitive : meshRef.primitives) {
        ...
        if (matIndex < static_cast<int32_t>(modelDescriptorSets.size())) {
          vkCmdBindDescriptorSets(
              ...
              1, &modelDescriptorSets[matIndex][currentFrame], 0, nullptr);
        }
...
void VulkanApplication::createTAAImages() {
  ...
  // Helper lambda to create image + view
  auto createImage = [&](VkImage &image, VkDeviceMemory &memory,
                         VkImageView &view, VkImageUsageFlags usage) {
    VkImageCreateInfo imageInfo{};
    imageInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
    imageInfo.imageType = VK_IMAGE_TYPE_2D;
    imageInfo.extent.width = extent.width;
...
void VulkanApplication::cleanupTAAPipeline() {
  ...
  if (taaPipelineLayout != VK_NULL_HANDLE) {
    ...
    taaPipelineLayout = VK_NULL_HANDLE;
  }
  ...
  if (taaDescriptorSetLayout != VK_NULL_HANDLE) {
    vkDestroyDescriptorSetLayout(device->getDevice(), taaDescriptorSetLayout,
                                 nullptr);
    taaDescriptorSetLayout = VK_NULL_HANDLE;
  }
}
...
void VulkanApplication::createGraphicsPipeline() {
  ...
  graphicsPipeline =
      new VulkanPipeline(device, "Shaders/shader.vert.spv",

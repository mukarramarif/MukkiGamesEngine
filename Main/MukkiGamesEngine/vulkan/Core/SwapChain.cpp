...
void VulkanSwap::initSwap(Device& device, const VkSurfaceKHR& vkSurface, GLFWwindow* window)
{
	this->window = window;
...
void VulkanSwap::createImageViews()
{
	if (!devicePtr) {
		throw std::runtime_error("Device pointer not set! Call initSwap first.");
	}
...
void VulkanSwap::createFramebuffers(VkRenderPass renderPass, VkImageView depthImageView)
{
	if (!devicePtr) {
		throw std::runtime_error("Device pointer not set! Call initSwap first.");
	}
	...
	if (swapChainImageViews.empty()) {
		throw std::runtime_error("Image views not created! Call createImageViews first.");
	}
...
void VulkanSwap::resizeSwapChain(Device& device, const VkSurfaceKHR& vkSurface, GLFWwindow* window)
{
...
SwapChainSupportDetails VulkanSwap::querySwapChainSupport(VkPhysicalDevice device)
{
...
VkSurfaceFormatKHR VulkanSwap::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
...
VkExtent2D VulkanSwap::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities, GLFWwindow* window) {
